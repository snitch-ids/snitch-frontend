...
pub fn Nav() -> Html {
    ...
    let (authentication_route, authentication_label) = if state.is_authenticated() {
        (Route::Logout, "Logout")
    } else {
        ...
        <nav class="border-gray-200 px-2 px-4 py-2.5 rounded bg-transparent">
              <ul class="text-sm font-medium bg-transparent dark:border-gray-700">
                ...
                if state.is_authenticated() {
                    ...
                    <li class="navbar-link">
                        <Link<Route> to={Route::UserDetailsComponent}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 48 48" stroke-width="3.0" stroke="currentColor" class="w-6 h-6 inline-block">
